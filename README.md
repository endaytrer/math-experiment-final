...
由于反射角仅仅和入射角相关, 所以放缩圆的大小不会影响结果. 故在极坐标系中建立圆$\rho=1$. 同样, 结果和发射点与空隙的绝对位置无关, 但和两者的相对位置有关, 故固定小球的发射点位于$(\rho,\theta)=(1,0)$处, 并规定空隙中心点位置在于$\theta_0$, 大小为$2\delta$, 即空隙可表示为$\{(1, \theta)|\theta\in\left[ \theta_0-\delta, \theta_0+\delta \right]\}$. 以逆时针为正方向, 质点发射的角度和$\theta=\frac\pi{2}$的夹角为$\phi$, $\phi\in\left(0, \pi\right)$.
...
\end{cases}
$$
不考虑空隙此时可以绘制出由15次反射, $\phi$从$0$变化到$\pi$时的反射模式.
